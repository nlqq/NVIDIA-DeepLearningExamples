...
if [ "X$distro" == "X" -a $deb -eq 0 ]; then
	...
	case "$distro_rpm" in
		...
		distro=$(echo $distro_rpm | sed -r -e 's/(redhat|centos|sl)-release-([0-9])\.([0-9]+).*/rhel\2.\3/')
		;;
		...
		distro=ol6.10
		;;
		oraclelinux-release-7.[0-8]-* | \
		oraclelinux-release-8.[0-1]-* | \
		__nosuchdistro__)
		distro=$(echo $distro_rpm | sed -r -e 's/oraclelinux-release-([0-9])\.([0-9]+).*/ol\1.\2/')
		;;
		bclinux-release-7-[3-6].*)
...
if [ $WITH_ETH_ONLY -eq 1 ]; then
	...
	bins=`find ../${BINDIR}/$MLNX_LIBS_REPO -name "*mlnx-en*" -o -name "*mlnx_en*" \

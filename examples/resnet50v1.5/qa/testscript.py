        ...
        for bspgpu in bspgpus:
            for databackend in databackends:
                ...
                if exit_code != 0:
                    ...
                    for m in metrics:
                        print(m)
                        table[m][ngpu][bspgpu][databackend] = stat.mean(raport['iter'][m][-2][skip_first_n:]) # -2 because there is a trailing [], and we want the last epoch
            ...
            for b in bs:
                for db in databackends:
                    ...
                    if (m in higher_is_better) and (result < PERF_THR * reference):
                        allright = False
                        print("Metric: {} NGPUs: {} BS: {} Data Backend: {} Result ( {} ) is more than {} times slower than reference ( {} )".format(
                            m, n, b, db, result, PERF_THR, reference))
                    if (m in lower_is_better) and (result * PERF_THR > reference):
                        allright = False
                        print("Metric: {} NGPUs: {} BS: {} Data Backend: {} Result ( {} ) is more than {:.1f} times higher than reference ( {} )".format(
